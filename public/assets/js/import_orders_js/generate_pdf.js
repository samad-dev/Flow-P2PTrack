...
function generatePDF(outputPath, json, pre) {
    // pre=false;
    console.log("pre", pre);
        ...
        for (let shift in result) {
            ...
            for (let haulier in result[shift]) {
                ...
                result[shift][haulier].forEach((vehicle, vehicleIndex) => {
                    ...
                    let order_eta = vehicle.trip[0].orders[0]['order_eta'].split(',')[0]; // "10/1/2024"
                    let order_etatime = vehicle.trip[0].orders[0]['order_eta'].split(',')[1]; // "10/1/2024"
                    ...
                    vehicle.trip.forEach((trip, index) => {
                    ...
                    vehicle.trip.forEach((trip, index) => {
                        ...
                        trip.compartments.forEach(comp => {
                            // Log order ID for debugging

                            // Find the corresponding compartment based on orderId
                            var order = trip.orders.find(order => order.orderId == comp.order_id);
                            ...
                            if (order) {
                                doc.fontSize(8).font('Helvetica')
                                    ...
                                    .text(truncateText(order.customerName), 165, amps)
                                    .text(order.shipTo, 320, amps)
                                    .text(order.orderId, 400, amps)
